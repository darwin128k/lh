 ...
 */
#define lh_assert_runtime(expr, ...) lh_runtime_assert(expr, __VA_ARGS__)

/**
 * @def lh_assert_runtime_if(expr, ...)
 * @brief Alias for ::lh_runtime_assert_if.
 *
 * Example usage:
 * @code{.c}
 * lh_assert_runtime_if(result < 0, lh_runtime_error_code_invalid_argument, "bad result");
 * @endcode
 *
 * @see lh_runtime_assert_if
 */
#define lh_assert_runtime_if(expr, ...) lh_runtime_assert_if(expr, __VA_ARGS__)

/**
 * @def lh_assert_runtime_ifn(expr, ...)
 ...
 */
#define lh_assert_runtime_ifn(expr, ...) lh_runtime_assert_ifn(expr, __VA_ARGS__)
