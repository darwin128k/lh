 ...
 * The name *raw* refers to a plain C array, not an lh container abstraction.
 */

#ifndef LH_UTIL_ARRAY_RAW_H
#define LH_UTIL_ARRAY_RAW_H

/**
 * @def lh_array_raw_get_size(x)
 ...
 * @code{.c}
 * int buf[10];
 * lh_array_raw_get_size(buf)   // → 10
 * @endcode
 */
#define lh_array_raw_get_size(x) (sizeof(x) / sizeof(x[0]))

/**
 * @def lh_array_raw_get_begin(x)
 ...
 *
 * Same preconditions as ::lh_array_raw_get_size:
 ...
 * @code{.c}
 * int buf[10];
 * for (int *p = lh_array_raw_get_begin(buf); p != lh_array_raw_get_end(buf); ++p) { }
 * @endcode
 */
#define lh_array_raw_get_begin(x) (x)

/**
 * @def lh_array_raw_get_end(x)
 * @brief Pointer one past the last element of the array object @p x.
 *
 * Expands to `((x) + lh_array_raw_get_size(x))`.
 * The result has the same type as @p x after array decay (pointer to element type).
 * Same preconditions as ::lh_array_raw_get_size: @p x must be an array object, not a bare pointer.
 *
 * @param x Array object or subobject of array type.
 *
 * Example usage:
 ...
 * @endcode
 */
#define lh_array_raw_get_end(x) ((x) + lh_array_raw_get_size(x))

#endif // LH_UTIL_ARRAY_RAW_H
