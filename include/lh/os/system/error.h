 ...
 *
 * ::lh_os_system_error_format is a pure, stateless formatter: given a code,
 * it writes the OS message into a buffer the caller owns. ::lh_os_system_error_capture
 * is the convenience entry point most call sites want: it reads the native
 * last error (`GetLastError` / `errno`), formats it into a private
 * thread-local buffer this module owns, and stores code + a view of that
 * buffer as the current thread's last error via ::lh_os_set_last_error
 * (see the three-layer split documented in `lh/os.h`).
 *
 ...
 * the last-error slot — @p dest is entirely the caller's buffer, sized
 * however they like.
 *
...
/**
 * @brief Snapshot the native last error as the OS-layer last error on this
 *        thread (::lh_os_set_last_error).
 *
