/**
 * @file time.h
 * @brief Filesystem timestamp representation (::lh_fs_time_t).
 *
 ...
 * to this on the way in (see `lh_os_system_fs_time_from_filetime_ticks` in
 * `src/lh/os/system/win/fs/stat.c`). Signed so a value before 1970 (rare,
 * but some filesystems allow it) is representable rather than wrapping.
 *
 * Purely a documentation typedef (alias for ::lh_timestamp_t, not a
 * distinct type) — it exists so `lh_fs_stat_t`'s atime/mtime/ctime
