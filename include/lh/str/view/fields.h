 ...
 *
 * Injects the same pair of pointer fields as ::lh_memory_range_fields,
 * with a name that reflects non-owning string slice semantics
 * (half-open <tt>[first, second)</tt> like memory ranges).
 */
 ...
 * @brief Expands to the same member declarations as ::lh_memory_range_fields.
 *
 * Use inside a struct body when two pointers delimit a string view.
 * Equivalent to `lh_memory_range_fields(T)` and thus to `lh_interval_bounds_fields(T *)`.
 *
 ...
 * @endcode
 *
 * @see lh_memory_range_fields
 * @see lh_interval_bounds_fields
 */
#define lh_str_view_fields(T) lh_memory_range_fields(T)
