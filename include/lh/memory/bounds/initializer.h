 ...
 * Example usage:
 * @code{.c}
 * static lh_memory_bounds_t bounds = lh_memory_bounds_initializer(ptr, ptr + n);
 * @endcode
 *
 * @see lh_initializer
 * @see lh_memory_bounds_t
 * @see lh_memory_bounds_init
 */
#define lh_memory_bounds_initializer(begin, end) lh_initializer(begin, end)

/**
 * @def lh_memory_bounds_empty_initializer()
 * @brief Initializer with both endpoints null (uninitialized pattern).
 *
 * Expands to ::lh_memory_bounds_initializer(::lh_null, ::lh_null). Matches the cleared
 * state described for ::lh_memory_bounds_is_uninitialized.
 *
