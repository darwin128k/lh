 ...
 * @brief If the condition is false, throw with ::lh_runtime_error_code_null_pointer.
 *
 * Expands to ::lh_runtime_check(__VA_ARGS__, ::lh_runtime_error_code_null_pointer).
 * Because ::lh_runtime_check throws when the expression is **false**, a typical use
 * is to pass a non-null pointer test (e.g. a bare pointer) so a null reference triggers the throw.
 *
