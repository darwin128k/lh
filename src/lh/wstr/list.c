#include <lh/wstr/list.h>
#include <lh/assert.h>
#include <lh/index.h>
#include <lh/size.h>
#include <lh/util/addr.h>
#include <lh/util/ptr.h>

lh_vector_t *
lh_wstr_list_get_items(lh_wstr_list_t *self)
{
    lh_assert_runtime_ref(self);
    return lh_addr_of(self->items);
}

const lh_vector_t *
lh_wstr_list_get_items_as_const(const lh_wstr_list_t *self)
{
    lh_assert_runtime_ref(self);
    return lh_addr_of(self->items);
}

void
lh_wstr_list_init(lh_wstr_list_t *self)
{
    lh_vector_init(lh_wstr_list_get_items(self), sizeof(lh_wstr_t));
}

void
lh_wstr_list_clear(lh_wstr_list_t *self)
{
    lh_vector_t *items;
    lh_usize_t n;

    items = lh_wstr_list_get_items(self);
    n = lh_vector_get_size(items);
    while (n > 0U)
    {
        --n;
        lh_wstr_deinit(lh_ptr_cast(lh_wstr_t, lh_vector_get_ptr(items, n)));
    }
    lh_vector_clear(items);
}

void
lh_wstr_list_deinit(lh_wstr_list_t *self)
{
    lh_wstr_list_clear(self);
    lh_vector_deinit(lh_wstr_list_get_items(self));
}

lh_uindex_t
lh_wstr_list_push_back(lh_wstr_list_t *self, lh_wstr_view_t text)
{
    lh_wstr_t value;

    lh_wstr_init(lh_addr_of(value));
    lh_wstr_assign_view(lh_addr_of(value), text);
    /* Bytes of value (including its heap pointer) are copied into the new
       slot; ownership moves there, so value itself is not deinited. */
    return lh_vector_push_back(lh_wstr_list_get_items(self), lh_addr_of(value));
}

lh_uindex_t
lh_wstr_list_push_back_str(lh_wstr_list_t *self, const lh_wstr_t *value)
{
    return lh_wstr_list_push_back(self, lh_wstr_as_view(value));
}

void
lh_wstr_list_assign(lh_wstr_list_t *self, const lh_wstr_list_t *other)
{
    lh_usize_t n;
    lh_uindex_t i;

    lh_assert_runtime_ref(other);
    if (self == other)
    {
        return;
    }
    lh_wstr_list_clear(self);
    n = lh_wstr_list_get_size(other);
    for (i = 0U; i < n; ++i)
    {
        lh_wstr_list_push_back_str(self, lh_wstr_list_get_as_const(other, i));
    }
}

void
lh_wstr_list_append(lh_wstr_list_t *self, const lh_wstr_list_t *other)
{
    lh_usize_t n;
    lh_uindex_t i;

    lh_assert_runtime_ref(other);
    n = lh_wstr_list_get_size(other);
    for (i = 0U; i < n; ++i)
    {
        lh_wstr_list_push_back_str(self, lh_wstr_list_get_as_const(other, i));
    }
}

void
lh_wstr_list_join(const lh_wstr_list_t *self, lh_wstr_t *out, lh_wchar_t sep)
{
    lh_usize_t n;
    lh_uindex_t i;

    n = lh_wstr_list_get_size(self);
    for (i = 0U; i < n; ++i)
    {
        if (i > 0U)
        {
            lh_wstr_push_back(out, sep);
        }
        lh_wstr_append_str(out, lh_wstr_list_get_as_const(self, i));
    }
}

lh_bool_t
lh_wstr_list_is_empty(const lh_wstr_list_t *self)
{
    return lh_vector_is_empty(lh_wstr_list_get_items_as_const(self));
}

lh_usize_t
lh_wstr_list_get_size(const lh_wstr_list_t *self)
{
    return lh_vector_get_size(lh_wstr_list_get_items_as_const(self));
}

lh_wstr_t *
lh_wstr_list_get(lh_wstr_list_t *self, lh_uindex_t index)
{
    return lh_ptr_cast(lh_wstr_t, lh_vector_get_ptr(lh_wstr_list_get_items(self), index));
}

const lh_wstr_t *
lh_wstr_list_get_as_const(const lh_wstr_list_t *self, lh_uindex_t index)
{
    return lh_ptr_cast(const lh_wstr_t,
                       lh_vector_get_ptr(lh_wstr_list_get_items_as_const(self), index));
}
