...
{
    return lh_memory_view_slice_get_flags(self);
}
...
lh_memory_view_contains_of(const lh_memory_view_t *self, const lh_ptr begin, const lh_ptr end)
{
    ...
    self_begin = lh_memory_view_get_begin_v(self);

    self_end = lh_memory_view_get_end_v(self);
...
{
    const lh_void *begin, *end;
    begin = lh_memory_view_get_begin(other);

    end = lh_memory_view_get_end(other);
    return lh_memory_view_contains_of(self, begin, end);
}
...
{
    return lh_ptr_deref(lh_ptr_cast(const lh_byte_t, lh_memory_view_get_ptr(self, offset)));
}
...
lh_memory_view_find_offset_from_ptr(const lh_memory_view_t *self, const lh_ptr ptr, lh_soffset_t offset,
                                    lh_uoffset_t *out)
{
    if (lh_ptr_is_null(ptr))
    {
        lh_ptr_deref(out) = lh_memory_view_get_offset_from_begin(self, lh_memory_view_get_ptr(self, offset));
                                   ...
                                   lh_soffset_t offset)
{
    ...
    code = lh_memory_view_find_offset_from_ptr(self, ptr, offset, lh_addr_of(result));
...
lh_memory_view_seek_ptr(const lh_memory_view_t *self, const lh_ptr ptr, lh_soffset_t offset)
{
    ...
    if (lh_math_ne(lh_memory_view_find_offset_from_ptr(self, ptr, offset, lh_addr_of(cur)), lh_runtime_error_code_ok))
    {
        return lh_null;
    }
    return lh_memory_view_get_ptr_from_begin(self, cur);
}
...
{
    const lh_bool_t is_begin_aligned = lh_memory_view_is_begin_aligned(self, align);
...
lh_memory_view_find(const lh_memory_view_t *self, const lh_memory_view_t *other)
{
    ...
    begin = lh_memory_view_get_begin_v(other);


    end = lh_memory_view_get_end_v(other);
...
LH_ATTRIBUTE_STATIC
lh_void
lh_memory_view_swap_v_other(lh_memory_view_t *self, lh_memory_view_t *other)
{
    lh_assert_runtime_ifn(lh_memory_view_is_valid(other),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_invalid_range));
...
lh_memory_view_swap_v(lh_memory_view_t *self, lh_memory_view_t *other)
{
    lh_assert_runtime_ifn(lh_memory_view_is_valid(self),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_invalid_range));
    lh_memory_view_swap_v_other(self, other);
}
