...
{
    return lh_memory_bounds_slice_get_flags(self);
}
...
lh_memory_bounds_contains(const lh_memory_bounds_t *self, const lh_memory_bounds_t *other)
{
    ...
    other_begin = lh_memory_bounds_get_begin(other);

    other_end = lh_memory_bounds_get_end(other);
...
{
    lh_assert_runtime_ifn(lh_memory_bounds_is_valid(other),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_invalid_range));
    return lh_memory_bounds_contains(self, other);
}
...
lh_memory_bounds_find_offset_from_ptr(const lh_memory_bounds_t *self, const lh_ptr ptr, lh_soffset_t offset,
                                      lh_uoffset_t *out)
{
    if (lh_ptr_is_null(ptr))
    {
        lh_ptr_deref(out) = lh_memory_bounds_get_offset_from_begin(self, lh_memory_bounds_get_ptr(self, offset));
    ...
    if (lh_math_is_negative(offset))
    {
        ...
        if (lh_interval_ropen_is_sub_overflow(ptr_offset, abs_offset, 0, size))
        {
    ...
    if (lh_interval_ropen_is_add_overflow(ptr_offset, lh_type_cast(lh_uoffset_t, offset), 0, size))
    {
                                     ...
                                     lh_soffset_t offset)
{
    ...
    code = lh_memory_bounds_find_offset_from_ptr(self, ptr, offset, lh_addr_of(result));
...
lh_memory_bounds_seek_ptr(const lh_memory_bounds_t *self, const lh_ptr ptr, lh_soffset_t offset)
{
    ...
    if (lh_math_ne(lh_memory_bounds_find_offset_from_ptr(self, ptr, offset, lh_addr_of(cur)), lh_runtime_error_code_ok))
    {
        return lh_null;
    }
    return lh_memory_bounds_get_ptr_from_begin(self, cur);
}
...
lh_memory_bounds_seek_value(const lh_memory_bounds_t *self, const lh_ptr ptr)
{
    ...
    lh_assert_runtime_ifn(lh_ptr_is_set(seek_ptr),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_null_dereference));
...
lh_memory_bounds_next_value(const lh_memory_bounds_t *self, const lh_ptr ptr)
{
    ...
    lh_assert_runtime_ifn(lh_ptr_is_set(next_ptr),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_null_dereference));
...
lh_memory_bounds_prev_value(const lh_memory_bounds_t *self, const lh_ptr ptr)
{
    ...
    lh_assert_runtime_ifn(lh_ptr_is_set(prev_ptr),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_null_dereference));
...
lh_memory_bounds_is_begin_aligned(const lh_memory_bounds_t *self, lh_usize_t align)
{
    ...
    lh_assert_runtime_ifn(lh_math_is_power_of_two(align),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_not_power_of_two));
...
lh_memory_bounds_take_first(lh_ptr begin, lh_ptr end, lh_usize_t n)
{
    ...
    {
        return lh_memory_bounds_make_empty();
    }

    const lh_memory_bounds_t base = lh_memory_bounds_make_v(begin, end);
    const lh_usize_t total = lh_memory_bounds_get_size(lh_addr_of(base));

    lh_assert_runtime_ifn(lh_math_le(n, total),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_out_of_range));
    return lh_memory_bounds_make_by_size(begin, n);
}
...
lh_memory_bounds_take_last(lh_ptr begin, lh_ptr end, lh_usize_t n)
{
    ...
    lh_ptr data = lh_ptr_sub_by_offset_unsafe(lh_void, end, n);
    return lh_memory_bounds_make_by_size(data, n);
}

lh_memory_bounds_t
lh_memory_bounds_make_from_begin(const lh_memory_bounds_t *self, lh_usize_t size)
{
    return lh_memory_bounds_take_first(lh_memory_bounds_get_begin(self),
                                       lh_memory_bounds_get_end(self), size);
}

lh_memory_bounds_t
lh_memory_bounds_make_from_end(const lh_memory_bounds_t *self, lh_usize_t size)
{
    return lh_memory_bounds_take_last(lh_memory_bounds_get_begin(self),
                                      lh_memory_bounds_get_end(self), size);
}
...
{
    lh_assert_runtime_ifn(lh_memory_bounds_contains_range(self, begin, end),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_out_of_range));
...
lh_memory_bounds_make_from_offset(const lh_memory_bounds_t *self, lh_uoffset_t offset,
                                  lh_usize_t size)
{
    const lh_usize_t total = lh_memory_bounds_get_size(self);
    ...
    return lh_memory_bounds_make_by_size(lh_memory_bounds_get_ptr_from_begin(self, offset), size);
}
...
lh_memory_bounds_drop_first(const lh_memory_bounds_t *self, lh_usize_t n)
{
    ...
    lh_assert_runtime_ifn(lh_math_le(n, total),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_out_of_range));
    return lh_memory_bounds_make_from_offset(self, n, lh_math_sub(total, n));
}
...
lh_memory_bounds_drop_last(const lh_memory_bounds_t *self, lh_usize_t n)
{
    ...
    lh_assert_runtime_ifn(lh_math_le(n, total),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_out_of_range));
    return lh_memory_bounds_make_from_begin(self, lh_math_sub(total, n));
}
...
lh_memory_bounds_trim(const lh_memory_bounds_t *self, lh_usize_t left, lh_usize_t right)
{
    ...
    return lh_memory_bounds_make_from_offset(self, left, lh_math_sub(rest, right));
}
