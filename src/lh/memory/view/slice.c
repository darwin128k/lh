...
lh_memory_view_slice_unpack_v(const lh_memory_view_slice_t *self, const lh_ptr *begin,
                              const lh_ptr *end)
{
    lh_runtime_check(lh_memory_view_slice_is_valid(self), lh_runtime_error_code_invalid_range);
...
lh_memory_view_slice_get_offset_from_begin(const lh_memory_view_slice_t *self, const lh_ptr ptr)
{
    ...
    lh_runtime_check(lh_memory_view_slice_is_valid_offset(self, offset),
...
lh_memory_view_slice_overlaps_v(const lh_memory_view_slice_t *self,
                                const lh_memory_view_slice_t *other)
{
    lh_runtime_check(lh_memory_view_slice_is_valid(other), lh_runtime_error_code_invalid_range);
                                         ...
                                         lh_soffset_t offset)
{
    ...
    lh_runtime_check_if(lh_interval_closed_is_add_overflow(
...
lh_memory_view_slice_make_v(const lh_ptr begin, const lh_ptr end)
{
    ...
    lh_runtime_check(lh_memory_view_slice_is_valid(lh_addr_of(slice)),
...
lh_memory_view_slice_take_first(const lh_ptr begin, const lh_ptr end, lh_usize_t n)
{
    ...
    lh_runtime_check(lh_math_le(n, total), lh_runtime_error_code_out_of_range);
...
lh_memory_view_slice_make_between(const lh_memory_view_slice_t *self, const lh_ptr begin,
                                  const lh_ptr end)
{
    lh_runtime_check(lh_memory_view_slice_contains_range(self, begin, end),
                     lh_runtime_error_code_out_of_range);
                                      ...
                                      lh_usize_t size)
{
    ...
    lh_runtime_check(lh_math_le(offset, total), lh_runtime_error_code_out_of_range);
    ...
    lh_runtime_check(lh_math_le(size, lh_math_sub(total, offset)),
                     lh_runtime_error_code_out_of_range);
...
{
    const lh_usize_t total = lh_memory_view_slice_get_size(self);
    lh_runtime_check(lh_math_le(left, total), lh_runtime_error_code_out_of_range);

    const lh_usize_t rest = lh_math_sub(total, left);
    lh_runtime_check(lh_math_le(right, rest), lh_runtime_error_code_out_of_range);
