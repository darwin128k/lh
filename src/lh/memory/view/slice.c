...
lh_memory_view_slice_get_direction(const lh_memory_view_slice_t *self)
{
    ...
    {
        if (lh_interval_closed_is_valid(lh_ptr_to_uaddr(self->first),
...
lh_memory_view_slice_get_size(const lh_memory_view_slice_t *self)
{
    lh_assert_runtime_ifn(lh_memory_view_slice_is_valid(self),
                          lh_runtime_error_make_by_code(lh_runtime_error_code_invalid_range));
    return lh_interval_closed_get_size(lh_ptr_to_uaddr(self->first),
                                       lh_ptr_to_uaddr(self->second));
}
...
lh_memory_view_slice_get_offset_from_begin(const lh_memory_view_slice_t *self, const lh_ptr ptr)
{
    ...
    lh_assert_runtime_ifn(lh_memory_view_slice_is_valid_offset(self, offset),
...
lh_memory_view_slice_contains_ptr(const lh_memory_view_slice_t *self, const lh_ptr ptr)
{
    ...
    begin = lh_memory_view_slice_get_begin_v(self);

    end = lh_memory_view_slice_get_end_v(self);
                                    ...
                                    const lh_ptr end)
{
    ...
    self_begin = lh_memory_view_slice_get_begin_v(self);

    self_end = lh_memory_view_slice_get_end_v(self);
...
{
    const lh_void *begin, *end;
    begin = lh_memory_view_slice_get_begin(other);

    end = lh_memory_view_slice_get_end(other);
    return lh_memory_view_slice_contains_range(self, begin, end);
}
...
{
    lh_assert_runtime_ref(self);
    return lh_ptr_eq(self->first, begin) && lh_ptr_eq(self->second, end);
}
...
lh_memory_view_slice_equals(const lh_memory_view_slice_t *self, const lh_memory_view_slice_t *other)
{
    ...
    return lh_memory_view_slice_equals_of(self, other->first, other->second);
}
...
lh_memory_view_slice_find(const lh_memory_view_slice_t *self, const lh_memory_view_slice_t *other)
{
    ...
    begin = lh_memory_view_slice_get_begin_v(other);


    end = lh_memory_view_slice_get_end_v(other);
...
{
    return lh_memory_view_slice_get_ptr_from_end(self, 0);
}
...
{
    return lh_ptr_deref(lh_ptr_cast(const lh_byte_t, lh_memory_view_slice_get_ptr(self, offset)));
}
