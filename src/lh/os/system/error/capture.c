...
#        error "lh/os/system/error/capture.c: LH_LIBRARY_OPTION_OS_WERROR is Windows-only for now"
#    endif
#endif
...
#if LH_COMPILER_OS == LH_COMPILER_OS_WINDOWS
    code = lh_cast_static(lh_os_system_error_code_t, GetLastError());
#else
    code = lh_cast_static(lh_os_system_error_code_t, errno);
#endif
