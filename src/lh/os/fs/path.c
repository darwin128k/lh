...
lh_os_fs_path_is_empty(const lh_os_fs_path_t *self)
{
    return lh_cast_static(lh_bool_t,
                          lh_math_eq(lh_os_fs_path_get_root_kind(self), lh_os_fs_path_root_kind_none) &&
...
{
    return lh_cast_static(lh_bool_t,
                          lh_math_ne(lh_os_fs_path_get_root_kind(self), lh_os_fs_path_root_kind_none));
}

lh_bool_t
lh_os_fs_path_is_root(const lh_os_fs_path_t *self)
{
    return lh_cast_static(lh_bool_t, lh_os_fs_path_is_absolute(self) &&
                                     lh_math_eq(lh_os_fs_path_get_segment_count(self), 0U));
}
...
lh_os_fs_path_to_str(const lh_os_fs_path_t *self, lh_str_t *out)
{
    ...
    if (lh_math_eq(lh_os_fs_path_get_root_kind(self), lh_os_fs_path_root_kind_drive))
    {
        lh_str_push_back(out, lh_os_fs_path_get_root_drive(self));
        lh_str_push_back(out, lh_char_map_colon);
        lh_str_push_back(out, sep);
    }
    else if (lh_math_eq(lh_os_fs_path_get_root_kind(self), lh_os_fs_path_root_kind_posix))
    {
        lh_str_push_back(out, sep);
    }
...
lh_os_fs_path_join(lh_os_fs_path_t *self, const lh_os_fs_path_t *dir, const lh_os_fs_path_t *name)
{
    ...
    if (lh_math_eq(lh_os_fs_path_get_segment_count(name), 0U))
    {
        lh_os_fs_path_clear(self);
        return lh_bool_false;
    }
