...
lh_os_fs_attr_hidden_from_path(const lh_os_fs_path_t *path)
{
    ...
    n = lh_vector_get_size(lh_os_fs_path_get_parts_as_const(path));
    if (n == 0U)
    {
        return 0U;
    }
...
lh_os_fs_file_stat(const lh_os_fs_file_t *self, lh_os_fs_stat_t *out)
{
    ...
    {
        lh_os_set_last_error(lh_os_error_code_not_open,
...
#if LH_COMPILER_OS == LH_COMPILER_OS_WINDOWS
    {
        ...
        if (!GetFileInformationByHandle(lh_cast_reinterpret(HANDLE, self->handle),
                                        lh_addr_of(info)))
        {
            lh_os_capture_last_error();
            return lh_bool_false;
        ...
        lh_os_fs_stat_fill_from_win(out, info.dwFileAttributes, info.nFileSizeHigh,
                                    info.nFileSizeLow, lh_addr_of(info.ftLastAccessTime),
                                    lh_addr_of(info.ftLastWriteTime),
                                    lh_addr_of(info.ftCreationTime), lh_bool_false, 0U);
        return lh_bool_true;
    }
#else
    {
        ...
        if (fstat(lh_cast_static(int, self->handle), lh_addr_of(info)) != 0)
        {
            lh_os_capture_last_error();
            return lh_bool_false;
        }
        return lh_os_fs_stat_fill_from_unix(out, lh_addr_of(info), 0U);
    }
#endif
}
