...
lh_os_net_socket_open(lh_os_net_socket_t *self, lh_os_net_socket_type_t type)
{
    ...
    lh_assert_runtime_ref(self);
    lh_assert_runtime_if(lh_math_ne(type, lh_os_net_socket_type_tcp) &&
                             lh_math_ne(type, lh_os_net_socket_type_udp),
                         lh_runtime_error_make_by_code(lh_runtime_error_code_invalid_argument));

    if (lh_math_eq(type, lh_os_net_socket_type_tcp))
    {
    ...
    if (lh_math_eq(handle, LH_OS_NET_SOCKET_HANDLE_INVALID))
    {
...
lh_os_net_socket_close(lh_os_net_socket_t *self)
{
    ...
    if (lh_math_ne(self->handle, LH_OS_NET_SOCKET_HANDLE_INVALID))
    {
#if LH_COMPILER_OS == LH_COMPILER_OS_WINDOWS
...
{
    lh_assert_runtime_ref(self);
    return lh_cast_static(lh_bool_t, lh_math_ne(self->handle, LH_OS_NET_SOCKET_HANDLE_INVALID));
}
                        ...
                        lh_net_ip4_socket_addr_t *peer)
{
    ...
    lh_assert_runtime_if(lh_math_ne(client->handle, LH_OS_NET_SOCKET_HANDLE_INVALID),
                         lh_runtime_error_make_by_code(lh_runtime_error_code_invalid_argument));
...
lh_os_net_socket_send(lh_ptr context, const lh_ptr buf, lh_usize_t size)
{
...
#else
    {
        lh_ssize_t result = lh_cast_static(
            lh_ssize_t, send(lh_cast_static(lh_os_net_native_handle_t, self->handle), buf,
...
lh_os_net_socket_recv(lh_ptr context, lh_ptr buf, lh_usize_t size)
{
    ...
    {
        lh_ssize_t result = lh_cast_static(
            lh_ssize_t, recv(lh_cast_static(lh_os_net_native_handle_t, self->handle), buf,
                             lh_cast_static(size_t, size), 0));
        if (lh_math_is_negative(result))
        {
            lh_os_system_error_capture();
