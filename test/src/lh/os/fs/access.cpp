...
TEST(os_fs_access, empty_path_fails)
{
    ...
    EXPECT_EQ(lh_os_fs_is(lh_addr_of(path), lh_os_fs_access_readable), lh_bool_false);
...
TEST(os_fs_access, written_file_is_readable_and_writable)
{
    lh_os_fs_path_t path;
    ...
    ASSERT_EQ(lh_os_fs_file_open(lh_addr_of(file), lh_addr_of(path), lh_os_fs_file_mode_write),
              lh_bool_true);
    EXPECT_EQ(lh_os_fs_file_is(lh_addr_of(file), lh_os_fs_access_writable), lh_bool_true);
    EXPECT_EQ(lh_os_fs_file_is(lh_addr_of(file), lh_os_fs_access_readable), lh_bool_false);
    EXPECT_EQ(lh_os_fs_file_is(lh_addr_of(file), lh_os_fs_access_executable), lh_bool_false);
    lh_os_fs_file_deinit(lh_addr_of(file));

    EXPECT_EQ(lh_os_fs_is(lh_addr_of(path), lh_os_fs_access_readable), lh_bool_true);
    EXPECT_EQ(lh_os_fs_is(lh_addr_of(path), lh_os_fs_access_writable), lh_bool_true);

    lh_os_fs_path_deinit(lh_addr_of(path));
    (void)std::remove("lh_os_fs_access.bin");
}

TEST(os_fs_file, is_closed_is_false)
{
    lh_os_fs_file_t file;
