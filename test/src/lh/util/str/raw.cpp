...
TEST(str_raw_trim_death, null_arguments) {
    ...
    LH_EXPECT_DEATH(lh_str_raw_trim_custom(reinterpret_cast<lh_str_ptr>(lh_null), lh_str_raw_len(s),
                                           ws, 1));

    LH_EXPECT_DEATH(lh_str_raw_ltrim_custom(s, lh_str_raw_len(s),
                                            reinterpret_cast<lh_str_cptr>(lh_null), 1));
}
