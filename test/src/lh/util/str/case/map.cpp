...
{
    lh_char_t buf[] = {'H', 'i', '!', '\0'};
    lh_str_cptr end = lh_str_ptr_to_lower(buf, 3);
    EXPECT_EQ(end, buf + 3);
    EXPECT_EQ(buf[0], 'h');
    EXPECT_EQ(buf[1], 'i');
    EXPECT_EQ(buf[2], '!');
...
{
    lh_char_t buf[] = {'a', 'B', '\0'};
    lh_str_cptr end = lh_str_ptr_to_upper(buf, 2);
    EXPECT_EQ(end, buf + 2);
    EXPECT_EQ(buf[0], 'A');
    EXPECT_EQ(buf[1], 'B');
}
...
{
    lh_char_t buf[] = {'X', 'Y', '\0'};
    lh_str_cptr end = lh_str_ptr_to_lower(buf, 0);
    EXPECT_EQ(end, buf);
    EXPECT_EQ(buf[0], 'X');
}
...
{
    lh_char_t buf[] = {'M', '\0'};
    lh_str_ptr_to_lower(buf, 1);
    EXPECT_EQ(buf[0], 'm');
    lh_str_ptr_to_upper(buf, 1);
    EXPECT_EQ(buf[0], 'M');
}
...
TEST(str_raw_to_lower_upper, digits_unchanged)
{
    ...
    lh_str_ptr_to_lower(buf, 3);
    EXPECT_EQ(buf[0], '1');
    EXPECT_EQ(buf[1], 'a');
    EXPECT_EQ(buf[2], 'b');
    lh_str_ptr_to_upper(buf, 3);
    EXPECT_EQ(buf[0], '1');
    EXPECT_EQ(buf[1], 'A');
    EXPECT_EQ(buf[2], 'B');
}
