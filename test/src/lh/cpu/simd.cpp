...
TEST(cpu_simd, sse2_is_always_true_on_x86_64)
{
    ...
    // it, so lh_cpu_has_sse2 must report true unconditionally here, with no real
    // CPUID check involved.
    EXPECT_EQ(lh_cpu_has_sse2(), lh_bool_true);
}
#endif
