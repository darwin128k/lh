 ...
 *
 * LH_LIBRARY_OPTION_RUNTIME_TERMINATE_USE_STDLIB is checked because it selects the **compile-time
 * default** for lh_runtime_terminate(): ON → initial handler is abort(), which EXPECT_DEATH can
 * rely on; OFF → initial handler is null, calling terminate on failed checks is undefined — these
 * tests are disabled. That does not prove anything at runtime: lh_runtime_terminate_set() can
 * replace abort() with another function; then EXPECT_DEATH may fail unless tests restore abort()
 * or otherwise ensure the process still dies in a way the death-test harness expects.
 */
